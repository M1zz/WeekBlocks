...
## 맥에서 아이폰 앱 권유 (2026-09-26)
      - IPhoneCompanionView.swift: '짜는 건 맥에서, 확인은 아이폰에서' 소개 장 + App Store QR·링크 보내기
      - 설정 맨 위(언어 아래) 한 줄 권유. 닫거나 소개를 읽으면 사라지고, '정보' 칸에 소개로 가는 길은 남는다
- [x] 구현·빌드
- [ ] 앱에서 눈으로 확인: QR을 아이폰 카메라로 비춰 App Store가 열리는지
- [x] 쓰는 중 권유: 주 화면 배너 (IPhoneNudgeBanner). 계획 블록 5개 이상 + 다른 기기 기록이 없을 때,
      다음 한 걸음·겹침 배너가 없을 때만. '알아보기' → 소개 시트, ✕ → 다시 안 뜸
- [ ] 앱에서 눈으로 확인: 배너가 뜨는 때, 이미 아이폰 쓰는 계정에서 안 뜨는지

## 종일 일정은 시간을 안 잡아먹는다 (2026-09-26, 레딧)
      - 캘린더 종일 일정(기념일 등)이 1시간짜리로 들어와 자유 시간을 깎던 것. 이제 '종일' 블록:
        `startHour = PlanBlock.allDayHour(-2)`, 길이 0. **새 필드 없음** — 아이폰은 '시각 없음·0시간'으로 읽는다
      - 블록으로 보기: 요일 머리 밑에 따로, 칩에 '종일'. 일간: 머리 밑 '종일' 줄. 시간축 자 위에는 안 선다
      - 편집기 '시간'에 '종일' 토글. 종일 블록을 자 위나 칩 사이에 놓으면 1시간짜리 시각 일정이 된다
      - 이미 1시간으로 들어온 종일 일정은 다음 캘린더 가져오기 때 종일로 바뀐다
- [x] 구현·빌드
- [ ] 앱에서 눈으로 확인: 캘린더 가져오기 후 종일 줄, 남은 시간 숫자가 안 줄어드는지
- [ ] iOS '욕망의 무지개'에서 startHour -2 블록을 맨 위 '종일'로 그리기 (지금은 시간대 자리에 0분으로 선다)
- [ ] 종일 줄 위로 시각 블록을 끌어 놓으면 종일로 바꾸기

## 앱 안에서 언어 고르기 (2026-09-25, 레딧)
      - AppLanguage.swift: System / English / 한국어. 설정 맨 위 + 앱 이름 메뉴의 'Language · 언어'.
        고르면 이 앱의 AppleLanguages에 적고 곧바로 다시 연다 (시스템 설정의 앱별 언어와 같은 자리)
- [ ] 앱에서 눈으로 확인: English 고르고 다시 열리는지, System으로 되돌아오는지 (자동 조작이 안 돼 못 봄)

## 한 주를 일요일부터 (2026-09-25, 레딧)
      - 설정 '주 시작' 월요일/일요일 (WeekStartSetting). **보이는 차례만** 바뀌고 기록은 ISO 주 그대로 —
        아이폰과 기존 기록이 안 깨진다. 일요일 시작 화면의 맨 앞 일요일은 앞 ISO 주의 일요일을 읽고 쓴다
      - 따라오는 곳: 블록으로 보기·시간축·일간 요일 줄·주 범위 글자·루틴 요일 단추·회고·지난 요일 판정
- [ ] 앱에서 눈으로 확인: 일요일 시작으로 바꾸고 일요일 칸에 끌어 놓기·옮기기·루틴 삭제(오늘만)
- [ ] 아직 ISO(월~일) 그대로인 곳: 다른 주 계획 가져오기(WeekCopyView), 회고 추세, 일정 공유 스냅숏,
      캘린더 가져오기(보이는 일요일은 앞 주를 볼 때 들어온다)
- [ ] iOS에도 같은 설정을 — 지금은 맥에만 (UserDefaults, 기기별)

## 모르는 말을 쓰는 사람에게 한국어가 뜨던 것 (2026-09-25, 레딧)
      - 개발 언어 ko → en (project.yml). 독일어 전용 맥에서 한국어로 뜨던 원인
      - 원문이 한국어라 ko 표가 안 만들어져 한국어 사용자에게 영어가 뜨는 것을 막으려고
        빌드마다 scripts/source-language-strings.py 가 ko.lproj/Localizable.strings 를 만든다 (앱·공유 확장)
- [ ] iOS '욕망의 무지개'도 개발 언어 확인 (아이폰은 영어로 떴으니 이미 맞을 수 있다)

## 겹친 시간은 누구 몫인가 (2026-09-25, 레딧 피드백)
      - **OverlapRule.swift**: '바깥 일정 기준'(기본, 지금까지) / '안쪽 일정 기준'(끼니·계획만큼 고정 루틴에서 뺌).
        설정 '겹친 시간 세기'에 그림 예시, 이번 주에 실제 겹침이 생기면 배너가 그 일정 이름으로 한 번 묻는다
      - 주간 요약 '고정 루틴'·'남은 자유 시간'이 겹친 시간을 두 번 빼던 것을 요일별 합집합으로 고침
- [ ] 앱에서 눈으로 확인: 배너 두 선택지 그림, 블록으로 보기 칩 길이, 일간 알약 옆 길이
- [ ] 점심 길이를 날마다 다르게 (지금은 끼니 한 회 길이가 루틴에 하나뿐)
- [ ] iOS에도 같은 기준(`overlapRule`)을 — 기기마다 따로 저장된다(UserDefaults)
- [ ] 루틴 삭제(오늘만) 무반응: 중복 배치 전부 숨기게 고침 → 실제로 되는지 확인
...
## 완료 (2026-09-22) — 시간 알약 하나로 · 끌어서 프로젝트로·할 일 안으로

- [x] **`DurationPicker`** (새 파일, XcodeGen 으로 프로젝트에 넣음 — pbxproj 4줄만 늘었다).
      누르면 15분·30분·45분·1시간·1시간 30분·2·3·4시간 버튼, 누르면 닫힌다. 목록에 없는 값은
      −/+(←/→)로 15분씩. 시간을 정하던 다섯 자리를 모두 이것으로:
      블록 편집기('길이 (h)' 소수 칸+▲▼ → '얼마나 걸릴지'), 할 일 상세 머리('다 하면'),
      단계 줄('0.5 h' 칸), 프로젝트 창 할 일 줄, 여러 개 한꺼번에 고치기.
- [x] **할 일 카드를 끌어**
      - 프로젝트 **묶음**(머리글·빈 곳)이나 **프로젝트 칩** 위에 놓으면 그 프로젝트로 ('프로젝트 없음' 묶음이면 뗀다).
        칩도 받는 까닭: 할 일이 아직 없는 프로젝트는 묶음이 안 선다.
      - **다른 카드** 위에 놓으면 그 할 일 **안의 하위 할 일**로 ("안으로 넣기" 표시). 단계 적기와 같은
        규칙 — 분류·주는 부모를 따르고 프로젝트는 뗀다(최상위 것을 따른다), `rollUp`, 제 단계는 딸려 온다.
      - 요일에서 끌어 내린 **블록**이 카드·묶음 위에 떨어져도 예전처럼 할 일로 되돌린다(안쪽이 먼저 받으므로).
- [ ] 알아 둘 것: 단계가 없던 할 일에 처음 하위 할 일을 넣으면 그 할 일의 시간은 **하위들의 합**이 된다
      (원래 적어 둔 시간은 셈에서 빠진다). 할 일 상세에서 첫 단계를 적을 때와 같은 기존 규칙.
- [ ] 하위로 넣은 것을 다시 꺼내는 끌기는 없다 — 할 일 상세에서 단계를 지우거나 옮겨야 한다.

## 완료 (2026-09-22) — 계획 블록 알약에 아이콘, 누르면 바뀐다 (일간)

- [x] 알약 속이 제목 첫 글자("밥먹기" → "밥")였다. 이제 SF Symbol 아이콘 — 두루 쓰이는 50개 중에서.
      `PlanBlock.symbol`: 정해 둔 것(`iconName`)이 없으면 **만든 시각**을 FNV-1a 로 섞어 늘 같은 하나를 뽑는다.
      ⚠️ `dragToken`(= persistentModelID)은 저장 전후·기기마다 달라 씨앗이 못 되고, `hashValue`는 실행마다 달라진다.
- [x] **알약 아이콘을 누르면** 다른 아이콘을 뽑아 적는다(`shuffleSymbol`, 아이콘이 갈려 바뀌는 결).
      끌기는 알약 어디서든 그대로 — 단추를 얹지 않고, 끌기 제스처가 '안 움직였고 누른 자리가 아이콘'일 때만
      다르게 받는다. 아이콘 자리는 관찰하지 않는 상자(`GlyphFrames`)에 적는다(스크롤마다 다시 그리지 않게).
- [x] 다른 주 계획 가져오기는 아이콘도 따라간다.
- [ ] ⚠️ **새 CloudKit 칸 `CD_PlanBlock.CD_iconName`.** 프로젝트·nextAction 과 함께 Production 배포에 실려야 한다.
      표본 올리기(양쪽)가 채운다. 아이폰 PlanBlock 에도 칸을 넣었다(스키마만, 화면은 아직).
- [ ] 주간 시간축 띠·요일 칸 칩에는 아직 아이콘이 없다(글자만).

## 완료 (2026-09-22) — 프로젝트를 훑어보기 쉽게

- [x] 할 일 머리에 **📁 프로젝트** 단추 ('전체 보기' 옆). 전엔 `…` 메뉴 안에만 있었다.
- [x] 프로젝트 창의 첫 화면이 **한눈에 보기** — 프로젝트마다 카드 한 장(진행 막대·%·남은 N개·남은 시간·
      다음 할 일), 누르면 그 프로젝트로. 끝낸 프로젝트는 아래에 옅게. 어느 프로젝트에서 왔으면(머리글·칩)
      바로 그 프로젝트로 연다. 프로젝트를 지우면 한눈에 보기로 돌아간다.
- [x] 왼쪽 목록 줄마다 **가는 진행 막대**(끝낸 할 일 ÷ 전체, 최상위만 센다).
- [x] 할 일 목록의 프로젝트 묶음을 **접는다** — 머리글 앞 ▸. 접은 것은 기억한다(`backlog.collapsedProjects`).
      머리글의 나머지를 누르면 예전처럼 프로젝트가 열린다. 접어도 머리글은 끌어다 놓기를 받는다.

## 완료 (2026-09-22) — 할 일 상세: 지금 할 단계가 줄기 바로 밑에

- [x] 단계는 순서대로 서고 끝낸 것도 제자리에 남아서, 앞을 끝낼수록 '지금' 줄이 아래로 밀렸다.
      끝낸 단계를 줄기 바로 밑 **"✓ 끝낸 단계 N개 ▸"** 한 줄로 접는다(`visibleRows`). 누르면 제 순서대로 펼친다.
      부모는 아래가 다 끝났을 때만 접으므로 보이는 단계의 부모는 늘 보인다 — 가지가 안 끊긴다.
      접힘 줄에도 세로선을 이어 긋는다.

## 완료 (2026-09-22) — 유동 루틴(끼니)의 시각을 매일 한 번에 바꾼다

유동 루틴의 기본 시각은 **어디에도 없었다** — 하루 활동 구간(07:30~19:30)에 고르게 나눈 계산값이라,
세 끼면 늘 07·13·19시. 시간표에서 끄는 것은 그 주 그 요일 한 칸(QuotaPlacement)만 바꿔서, 점심을
12시로 두려면 일곱 날을 끌어야 했고 다음 주면 도로 13시였다.
- [x] `Routine.sessionStartsRaw` ("7,12,19") — 회차별 기본 시각. nil 이면 예전처럼 고르게.
      `Routine.defaultSessionStart(i)` 하나를 시간표(`TimelineLayout`)와 편집기가 같이 본다.
- [x] **루틴 편집기 '매일 몇 시에'** — 지금 시간표에 서 있는 시각이 그대로 뜨고(계산값이든 적어 둔 것이든),
      회차마다 시각을 고치면 모든 날의 기본이 된다. 끝 시각도 옆에. '고르게 나누기로 되돌리기'.
      옛 문구 "정확한 요일·시간 없이 주 단위로만 추적합니다"는 이제 사실이 아니라 지웠다.
- [x] **끼니 알약 우클릭 → "이 시각을 매일 기본으로 (12:00)"** (일간·주간 시간축). 그날 따로 옮긴 기록은
